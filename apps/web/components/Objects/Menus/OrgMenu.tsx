      ...
      <header className="fixed inset-x-0 z-50 flex justify-center top-8 px-4 sm:px-0">
        <div className="relative w-full sm:w-3/4 md:w-2/3 lg:w-1/2">
          <div
            ...
            style={{
              background: showPride ? GRADIENT_PRIDE : GRADIENT,
              border: BORDER,
              boxShadow: SHADOW,
              filter: DROP_SHADOW,
...
function LeftRail({
  ...
  return (
    <div
      ...
      style={railStyle('left', showPride)}
    >
      {/* logo */}
      <Link href={getUriWithOrg(orgslug, '/')}>
        <div className="flex h-5 sm:h-7 md:h-8 lg:h-[26.8px] items-center">
...
function railStyle(side: 'left' | 'right', showPride = false): React.CSSProperties {
  return {
    background: side === 'left' && showPride
      ? GRADIENT_PRIDE
      : GRADIENT,
    border: BORDER,
    boxShadow: SHADOW,
